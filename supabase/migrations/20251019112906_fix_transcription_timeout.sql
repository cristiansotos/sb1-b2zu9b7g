/*
  # Fix Transcription Timeout Issue
  
  ## Problem
  The UPDATE operation on recordings table during transcription is timing out due to:
  - Complex RLS policies with expensive joins through 4 tables
  - Multiple UPDATE policies evaluating the same complex conditions
  - No efficient way to verify permissions for transcription updates
  
  ## Solution
  Create a dedicated function for updating transcription data that:
  - Uses SECURITY DEFINER to bypass RLS
  - Performs permission check once upfront
  - Updates the recording in a single efficient operation
  
  ## New Function
  - `update_recording_transcription`: Securely updates transcription data after permission check
  
  ## Security
  - Function validates user has permission before updating
  - Only allows updating specific transcription-related fields
  - Maintains data integrity and access control
*/

-- Function to update recording transcription (bypasses RLS for performance)
